...
impl VirtioBalloonDriver {
	...
	fn negotiate_features(
		...
		let common_features = self
			.com_cfg
			.control_registers()
			.negotiate_features(driver_features);

		if common_features.contains(driver_features) {
			// The device supports all our required features
			Ok(())
		} else {
			Err(VirtioBalloonError::IncompatibleFeatureSets { driver_features })
		}
	}
	...
	pub(crate) fn poll_events(&mut self, cx: &mut core::task::Context<'_>) {
	...
	unsafe fn deflate<B: Binning>(
		&mut self,
		talc: &mut Talc<DeflateBalloonOnOom, B>,
		num_pages_to_deflate: u32,
	) {
		assert!(
	...
	fn adjust_balloon_size(&mut self) {
		...
		if let Some(new_target_num_pages) = self.num_pages_changed() {
			if new_target_num_pages < self.num_in_balloon - self.num_pending_deflation {
				...
				self.inflate(&mut *ALLOCATOR.lock(), num_to_inflate, false);
	...
	pub unsafe fn deflate_for_oom<B: Binning>(
		&mut self,
		talc: &mut Talc<DeflateBalloonOnOom, B>,
...
/// Errors that can occur during the lifetime and initialization of the [`VirtioBalloonDriver`]
...
pub enum VirtioBalloonError {
	...
	#[error("Feature set: {driver_features:?}, is incompatible with the device's features")]
	IncompatibleFeatureSets { driver_features: virtio::balloon::F },
}
...
impl BalloonStorage {
	...
	pub fn allocate_chunks<B: Binning>(
		&mut self,
		talc: &mut Talc<DeflateBalloonOnOom, B>,
	...
	pub unsafe fn shrink_chunks<B: Binning>(
		&mut self,
		talc: &mut Talc<DeflateBalloonOnOom, B>,
...
impl BalloonAllocation {
	...
	pub fn try_allocate<B: Binning>(
		...
		let allocation_ptr = unsafe {
			talc.try_allocate(Self::layout(num_pages.try_into().expect(
	...
	pub unsafe fn shrink<B: Binning>(
		&mut self,
		talc: &mut Talc<DeflateBalloonOnOom, B>,
