...
impl VirtioNetDriver {
	...
	fn virtqueue_init(&mut self) -> Result<(), VirtioNetError> {
		...
		for i in 0..(self.num_vqs / 2) {
			if self.com_cfg.is_queue_ready(i.into()) {
				panic!("Queue {} is already ready", i);
			}

			if self.com_cfg.get_max_queue_size(i.into()) == 0 {
				panic!("Maximum queue is of queue {} is zero!", i);
			}
