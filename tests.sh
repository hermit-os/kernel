...
apt-get -qq update
apt-get install -y --allow-unauthenticated binutils-hermit newlib-hermit-rs pte-hermit-rs gcc-hermit-rs libhermit-rs
...
cd ..
mkdir -p tmp
dpkg-deb -R build/libhermit-rs-0.3.2-all.deb tmp
