...
fn getaddrinfo_node(
	...
	if ai_flags.contains(Ai::ADDRCONFIG) {
		#[cfg(feature = "net")]
		{
			let Some(mut guard) = NIC.try_lock() else {
				// FIXME: As per the man page, getaddrinfo can return EAGAIN, but might be inappropriate here
				return Err(Eai::Again);
			};
	...
	cfg_select! {
		feature = "dns" => resolve(nodename, ai_flags, ai_family, want_ipv4, want_ipv6),
		_ => {
