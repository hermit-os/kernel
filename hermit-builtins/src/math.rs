...
macro_rules! export {
    ($(fn $fn:ident($($arg:ident: $argty:ty),+) -> $retty:ty;)+) => {
        $(
			#[linkage = "weak_odr"]
            #[unsafe(no_mangle)]
            extern "C" fn $fn($($arg: $argty),+) -> $retty {
                ::libm::$fn($($arg),+)
            }
        )+
    };
...
macro_rules! export_out_param {
    ($(fn $fn:ident($($arg:ident: $argty:ty),+; $out:ident: $outty:ty) -> $retty:ty;)+) => {
        $(
			#[linkage = "weak_odr"]
            #[unsafe(no_mangle)]
            extern "C" fn $fn($($arg: $argty),+, $out: $outty) -> $retty {
...
#[linkage = "weak_odr"]
#[unsafe(no_mangle)]
unsafe extern "C" fn nan(arg: *const core::ffi::c_char) -> f64 {
	let _arg = arg;
	f64::NAN
}
...
#[linkage = "weak_odr"]
#[unsafe(no_mangle)]
unsafe extern "C" fn nanf(arg: *const core::ffi::c_char) -> f32 {
	let _arg = arg;
	f32::NAN
}
