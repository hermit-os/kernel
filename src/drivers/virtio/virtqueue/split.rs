...
impl DescrRing {
    fn push(&mut self, tkn: TransferToken) -> (Pinned<TransferToken>, u16, u16) {
        ...
        self.avail_ring.ring[*self.avail_ring.index as usize % self.avail_ring.ring.len()] = index as u16;
    ...
    fn poll(&mut self) {
        while self.read_idx != *self.used_ring.index {
            ...
            let tkn = unsafe {
                &mut *(self.ref_ring[used_elem.id as usize])
            };

            if tkn.buff_tkn.as_ref().unwrap().recv_buff.as_ref().is_some() {
                tkn.buff_tkn.as_mut().unwrap().restr_size(None, Some(used_elem.len as usize));
            }
...
impl SplitVq {
   /// Enables interrupts for this virtqueue upon receiving a transfer
    ...
    pub fn new(com_cfg: &mut ComCfg, 
        ...
        let _mem_len = align_up!(size as usize * core::mem::size_of::<Descriptor>(), BasePageSize::SIZE);
        let table_raw = (crate::mm::allocate(_mem_len, true).0 as *const Descriptor) as *mut Descriptor; 

        let descr_table = DescrTable {
            raw: unsafe {
                core::slice::from_raw_parts_mut(table_raw, size as usize)
            },
        };
        ...
        let avail_ring = unsafe {
            AvailRing {
                ...
                ring : core::slice::from_raw_parts_mut(avail_raw.offset(4) as *mut u16, size as usize),
        ...
        let used_ring = unsafe {
            UsedRing {
                ...
                ring : core::slice::from_raw_parts_mut((used_raw.offset(4) as *const _) as *mut UsedElem, size as usize),
