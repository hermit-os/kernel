...
impl FPUState {
	pub fn new() -> Self {
		let xsave_size = if supports_xsave()
			&& let Some(extended_state_info) = CpuId::new().get_extended_state_info()
		{
			extended_state_info.xsave_area_size_enabled_features() as usize
		} else {
			size_of::<XSaveLegacyRegion>()
		};

		info!("XSAVE area size: {xsave_size}");

		// Allocate a 64-byte aligned Vec
		let mut xsave_area = unsafe {
			//let n_units = xsave_size.div_ceil(size_of::<AlignToSixtyFour>());
			let n_units = xsave_size;
			let maybe_uninit_aligned = Box::<[AlignToSixtyFour]>::new_zeroed_slice(n_units);
			// SAFEFTY: [u8; 64] is valid when zero-initialized
			maybe_uninit_aligned.assume_init()
		};
