...
impl Dirent64 {
	/// Formats an Dirent64 (for debugging purposes)
	///
	/// # Safety
	/// The bytes following the `d_name` must form a valid zero terminated `CStr`. Else we have an out-of-bounds read.
	#[allow(dead_code)]
	unsafe fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
