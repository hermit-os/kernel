...
fn setup_msix(
	...
	let (base_address, _) = device.memory_map_bar(capability.table_bar(), true)?;
...
pub(crate) fn init_device(
	...
	// BAR0 holds the memory-mapped registers.
	let mut mapped = None;
	for i in 0..MAX_BARS {
		match device.get_bar(i.try_into().unwrap()) {
			Some(Bar::Memory32 { .. }) | Some(Bar::Memory64 { .. }) => {
				mapped = device.memory_map_bar(i.try_into().unwrap(), true);
				break;
			}
			_ => continue,
		}
	}
	let (addr, size) = mapped.ok_or(DriverError::InitIxgbeDevFail(IxgbeError::NoBar))?;
	let regs = Regs {
	...
	#[cfg(target_arch = "x86_64")]
	let irq = setup_msix(device, &regs, handlers).or(irq);
