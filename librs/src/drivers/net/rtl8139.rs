...
use scheduler;
use drivers::net::{NETWORK_TASK_ID, networkd, NET_SEM};
...
extern "C" fn rtl8139_thread(arg: usize) {
	...
	unsafe {
		...
		info!("Found RTL8139 at iobase 0x{:x} (irq {})", IOBASE, adapter.irq);
	}
	...
	unsafe {
		...
		outb(IOBASE + CONFIG1, (inb(IOBASE + CONFIG1) & !(CONFIG1_DVRLOAD | CONFIG1_LWACT)) | CONFIG1_DVRLOAD);
		 ...
		 */
		 outl(IOBASE + RCR, RCR_MXDMA2|RCR_MXDMA1|RCR_MXDMA0|RCR_AB|RCR_AM|RCR_APM|RCR_AAP); // The WRAP bit isn't set!
	...
	debug!("Allocate TxBuffer at 0x{:x} and RxBuffer at 0x{:x}", txbuffer, rxbuffer);
	...
	unsafe {
		...
		for i in 0..NO_TX_BUFFERS {
			outl(IOBASE + TSAD0, virt_to_phys(txbuffer+i*TX_BUF_LEN).try_into().unwrap());
		}
		...
		info!("RTL8139: CR = 0x{:x}, ISR = 0x{:x}, speed = {} mbps",
			inb(IOBASE + CR), inw(IOBASE + ISR), speed);
	}
...
extern "x86-interrupt" fn rtl8139_irqhandler(_stack_frame: &mut ExceptionStackFrame) {
	...
	unsafe {
		...
		while isr_contents != 0 {
			...
			outw(IOBASE + ISR, isr_contents & (ISR_RXOVW|ISR_TER|ISR_RER|ISR_TOK|ISR_ROK));
	...
	fn receive(&'a mut self) -> Option<(Self::RxToken, Self::TxToken)> {
		...
		if (cmd & CR_BUFE) != CR_BUFE {
			...
			if header & ISR_ROK == ISR_ROK {
				...
				error!("RTL8192: invalid header 0x{:x}, rx_pos {}\n", header, self.rxpos);
			}
		}
	
		POOLING.store(false, Ordering::SeqCst);
		// enable all known interrupts
		unsafe { outw(IOBASE + IMR, INT_MASK); }
		...
		where F: FnOnce(&mut [u8]) -> smoltcp::Result<R>
	{
		...
		unsafe {
			if TX_IN_USE[id] == true {
				...
				let mut buffer = core::slice::from_raw_parts_mut((self.txbuffer+id*TX_BUF_LEN) as *mut u8, len);
				let result = f(&mut buffer);
				if result.is_ok() {
					TX_IN_USE[id] = true;

					// send the packet
					outl(IOBASE + TSD0 as u16 + (4 * id as u16), len.try_into().unwrap()); //|0x3A0000);
				}
				result
			}
		}
...
impl Boards {
	pub const fn new(vendor_name: &'static str, device_name: &'static str, vendor_id: u16, device_id: u16) -> Self {
		Boards {
...
static BOARDS: [Boards;7]  = [
	...
	Boards::new("SMC", "SMC1211TX EZCard 10/100 (RealTek RTL8139)", 0x1113, 0x1211),
