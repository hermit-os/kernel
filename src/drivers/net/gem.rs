...
pub fn init_device(
	...
	unsafe {
		...
		for i in 0..RX_BUF_NUM {
			...
			let buffer = PhysAddr::from(
				DeviceAlloc.phys_addr((rxbuffer + u64::from(i * RX_BUF_LEN)).as_mut_ptr::<()>()),
			);
		...
		let rx_qbar: u32 = DeviceAlloc
			.phys_addr(rxbuffer_list.as_mut_ptr::<()>())
			.try_into()
			.unwrap();
		...
		for i in 0..TX_BUF_NUM {
			...
			let buffer = PhysAddr::from(
				DeviceAlloc.phys_addr((txbuffer + u64::from(i * TX_BUF_LEN)).as_mut_ptr::<()>()),
			);
		...
		let tx_qbar: u32 = DeviceAlloc
			.phys_addr(txbuffer_list.as_mut_ptr::<()>())
			.try_into()
			.unwrap();
