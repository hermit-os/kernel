//! Advanced Platform-Level Interrupt Controller (APLIC) driver for RISC-V.
//!
...
impl From<DeviceTreeInterruptType> for SourceMode {
	fn from(value: DeviceTreeInterruptType) -> Self {
		match value {
			DeviceTreeInterruptType::None => Self::Inactive,
...
struct AplicControlRegion {
	...
	#[access(ReadWrite)]
	sourcecfg: [SourceConfigRegister; 1023],
	#[access(NoAccess)]
...
pub(crate) struct Aplic {
	/// Control region of the APLIC, mapped into virtual memory
	control_region: VolatileRef<'static, AplicControlRegion>,
	/// Interrupt delivery control registers for each hart, mapped into virtual memory
	interrupt_delivery_control: VolatileRef<'static, InterruptDeliveryControlArray>,
	/// Number of implemented interrupt priority bits (IPRIOLEN)
	ipriolen: u8,
...
impl Aplic {
	...
	pub fn get_interrupt_source_mode(&mut self, irq_number: u16) -> SourceMode {
		let sourcecfg = unsafe {
			self.control_region.as_mut_ptr().sourcecfg().map(|slice| {
				slice
					.cast()
