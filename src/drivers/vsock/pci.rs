...
use crate::drivers::vsock::{FeatureSet, VirtioVsockDriver, VsockDevCfg};
...
impl VirtioVsockDriver {
	...
	pub(crate) fn init(
		...
		match drv.init_dev() {
			Ok(_) => info!(
				"Socket device with id {:x}, has been initialized by driver!",
				drv.get_dev_id()
			),
			Err(fs_err) => {
				drv.set_failed();
				return Err(VirtioError::VsockDriver(fs_err));
			}
		}
