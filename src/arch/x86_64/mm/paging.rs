...
use x86_64::structures::paging::{
	FrameAllocator, Mapper, OffsetPageTable, Page, PageTable, PhysFrame, Size1GiB, Size2MiB,
	Size4KiB, Translate,
};
...
fn split_page<S: PageSize>(page: Page<S>) {
	...
	let pt_page =
		PageAlloc::allocate(PageLayout::from_size(Size4KiB::SIZE as usize).unwrap()).unwrap();
	...
	let (start_addr, flags) = match table_explorer.translate(page.start_address()) {
		TranslateResult::Mapped { frame, flags, .. } => {
			let start_addr = match frame {
				MappedFrame::Size2MiB(frame) if S::SIZE == Size2MiB::SIZE => frame.start_address(),
				MappedFrame::Size1GiB(frame) if S::SIZE == Size1GiB::SIZE => frame.start_address(),
	...
	for (offset, entry) in pt.iter_mut().enumerate() {
		...
		entry.set_addr(start_addr + offset, flags);
	}
