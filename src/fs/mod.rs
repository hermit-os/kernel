...
pub(crate) trait VfsNode: core::fmt::Debug {
	...
	fn traverse_unlink(&self, _components: &mut Vec<&str>) -> Result<(), io::Error> {
		Err(io::Error::ENOSYS)
	}
...
impl Filesystem {
	...
	pub fn mkdir(&self, path: &str, mode: AccessPermission) -> Result<(), io::Error> {
	...
	pub fn opendir(&self, path: &str) -> Result<Arc<dyn ObjectInterface>, io::Error> {
		debug!("Open directory {}", path);
		Ok(Arc::new(DirectoryReader::new(self.readdir(path)?)))
	}

	/// List given directory
	pub fn readdir(&self, path: &str) -> Result<Vec<DirectoryEntry>, io::Error> {
		if path.trim() == "/" {
	...
	/// stat
	pub fn stat(&self, path: &str) -> Result<FileAttr, io::Error> {
	...
	/// lstat
	pub fn lstat(&self, path: &str) -> Result<FileAttr, io::Error> {
...
pub(crate) fn opendir(name: &str) -> Result<FileDescriptor, io::Error> {
...
pub fn file_attributes(path: &str) -> Result<FileAttr, io::Error> {
...
impl Metadata {
	...
	pub fn modified(&self) -> Result<SystemTime, io::Error> {
