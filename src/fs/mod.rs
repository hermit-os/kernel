...
impl Filesystem {
	...
	async fn traverse_prefinal<'a>(
		...
		components.next().ok_or(Errno::Nosys)?;
		let mut node = self.root.dup();

		while let Some(component) = components.next() {
			let mut tmp = components.clone();
			if tmp.next().is_none() {
				return Ok((node, component));
			}
	...
	/// Traverses a path, fully
	async fn traverse_fully(&self, path: &str) -> io::Result<Box<dyn VfsNode>> {
		let mut components = path.split('/');
		components.next().ok_or(Errno::Nosys)?;
		let mut node = self.root.dup();
	...
	pub fn readdir(&self, path: &str) -> io::Result<Vec<DirectoryEntry>> {
		block_on(
			async { self.traverse_fully(path).await?.readdir().await },
			None,
		)
	}
	...
	pub fn stat(&self, path: &str) -> io::Result<FileAttr> {
		...
		block_on(
			async { self.traverse_fully(path).await?.stat().await },
			None,
		)
	}
	...
	pub fn lstat(&self, path: &str) -> io::Result<FileAttr> {
		...
		block_on(
			async { self.traverse_fully(path).await?.lstat().await },
			None,
		)
	}
